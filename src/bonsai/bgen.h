...
void generateBonsai(glm::vec3 pos, std::vector<glm::vec3> &cubePositions,
                    int growth, int xdir, int zdir);
int checkCollision(std::vector<glm::vec3> cubePositions, glm::vec3 pos);
...
void generateBonsai(glm::vec3 pos, std::vector<glm::vec3> &cubePositions,
  ...
  if (growth == 0) // base case
    ...
    for (unsigned int i = 0; i < MAX_XZ_GROWTH; i++) {
      ...
      if (checkTouching(cubePositions, npos) > 2)
        return;
    ...
    /* recursive case */
    generateBonsai(npos, cubePositions, growth - 1, xdir, zdir);
  }
}
